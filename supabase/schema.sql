...
--
-- ALL Teammate C review changes applied (2026-09-13):
--   #1 root_node_id NON-NULL + deferred composite FK to a node in the SAME loop.
--   #2 composite (loop_id, id) FKs so edges / actions / approvals cannot cross loops.
--   #3 loop_source_events association table for durable Loop.source_event_ids provenance.
--   #4 non-empty source_apps; JSON shape checks (attachments array; others objects).
--   #5 static risk-flag checks: MEDIUM actions require approval; comms sends + external
--      Calendar update/cancel are forced to MEDIUM + requires_approval.
--   + RLS enabled with read-only policies (frontend can read/subscribe, cannot write).
--   + evidence_requirements added to the realtime publication.
--   + compilations table to persist assumptions / pending clarification across restart.
--   + idempotency comment corrected (unique key stops a duplicate ROW, not a re-execution).
--
-- TWO THINGS STILL NEEDING A TEAM DECISION (implemented conservatively, not blindly):
--   * RLS read policy is `using (true)` = demo-grade (any authenticated reader sees all
--     rows). Owner-scoped policies need the "real Supabase Auth UUID vs demo id" decision
--     the review raised; NO auth.users FK was added (review warned against doing it blindly).
--   * The #5 risk checks hardcode the comms action_type names. If a new send-type action is
--     added later, this CHECK must be updated or those inserts will be rejected.
--
